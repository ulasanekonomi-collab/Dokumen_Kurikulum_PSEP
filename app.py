...
with col1:
    ...
    st.markdown(f'<a href="{make_github_url(name_docx)}"><button style="width:100%; border-radius:10px; background-color:#0078d4; color:white; padding:10px; cursor:pointer;">📥 Download .DOCX</button></a>', unsafe_allow_html=True)

with col2:
    st.success("Versi Cetak (PDF)")
    name_pdf = "Draft 7 Revisi Kurikulum EP_2025_YS.pdf"
    st.markdown(f'<a href="{make_github_url(name_pdf)}"><button style="width:100%; border-radius:10px; background-color:#d13438; color:white; padding:10px; cursor:pointer;">📂 Buka .PDF</button></a>', unsafe_allow_html=True)
